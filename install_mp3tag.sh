#!/bin/bash
echo;echo;echo "  =========================";echo "  == Mp3Tag for Linux =="
...
echo " > This script will install Mp3Tag on your computer."
...
setupfile="mp3tagv326-x64-setup.exe"
else
prefx=".win32";warch="win32"
setupfile="mp3tagv326setup.exe"
fi
...
sudo cp -f ./files/mp3tag.png /usr/share/icons/hicolor/128x128/apps
sudo cp -f ./files/mp3tag.desktop "$HOME/.local/share/applications"
sudo cp -f ./files/mp3tag.sh "$HOME/$prefx"
sudo chmod +x "$HOME/$prefx/mp3tag.sh"
sudo sed -i "s|\$HOME|$HOME|g" "$HOME/.local/share/applications/mp3tag.desktop"
sudo sed -i "s|\.win64|$prefx|g" "$HOME/.local/share/applications/mp3tag.desktop"
sudo sed -i "s|\.win64|$prefx|g" "$HOME/$prefx/mp3tag.sh"
